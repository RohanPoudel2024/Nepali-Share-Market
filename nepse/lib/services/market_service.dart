...
class MarketService {
  ...
  // Add this method to your MarketService class
  Future<Response> getStockHistoricalData(String symbol) async {
    try {
      final response = await _apiClient.get('${ApiEndpoints.stockHistory}/$symbol');
      return response;
    } catch (e) {
      print('Error fetching stock historical data: $e');
      // Return a mock response to prevent crashes
      return Response(
        statusCode: 404,
        data: {'success': false, 'message': 'Failed to load historical data'},
